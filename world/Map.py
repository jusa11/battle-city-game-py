...
from configs.tile_confing import concrete_tile, forest_tile
from world.BricksTile import BricksTile
...
class Map(pygame.sprite.Sprite):
        ...
        for row_index, row in enumerate(level_1):
            for col_index, tile in enumerate(row):
                ...
                if tile == 2:
                    ...
                    self.busy_tiles.append((x // TILE_SIZE, y // TILE_SIZE))
                if tile == 3:
                    x = col_index * TILE_SIZE
                    y = row_index * TILE_SIZE
    ...
    def check_is_free_tile(self, coordinate):
        for tile in self.busy_tiles:
            if tile == coordinate:
                return False
        return True
